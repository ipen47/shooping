<template>
  <div class="user">
    <div class="head-page" v-if="isLogin">
      ...
      <div class="head-info">
        <div class="phone">{{ userDetail.mobile }}</div>
        <div class="vip"><van-icon name="diamond-o" /> 普通会员</div>
      </div>
    </div>
    <div class="head-page" v-else @click="login">
      <div class="hear-avatar">
        <van-image
          round
          width="100%"
    ...
    <div class="my-account">
      <div class="my-account-item" style="margin-left: 20px">
        <span>{{ userDetail.pay_money || 0 }}</span> <span>账户余额</span>
      </div>
...
export default {
  ...
  methods: {
    getAccountData() {
      //这里先用假数据
      this.userDetail = { mobile: "176xxxx0407", pay_money: 4784295.12 };
    },
    //登录
    login() {
      this.$router.replace({
